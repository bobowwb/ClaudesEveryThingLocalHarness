...
if (
  test('merge-mcp-config no-ops after all recommended servers are present', () => {
    ...
    try {
      ...
      assert.ok(!Object.prototype.hasOwnProperty.call(parsed.mcp_servers, 'memory'));
...
if (
  test('merge-mcp-config removes deprecated memory MCP in favor of MAMGA startup', () => {
    const tempDir = createTempDir('mcp-merge-memory-deprecated-');
    const configPath = path.join(tempDir, 'config.toml');
    const original = [
      '[mcp_servers.memory]',
      'command = "npx"',
      ...
      '[mcp_servers.github]',
      'command = "bash"',
      'args = ["-lc", "exec npx @modelcontextprotocol/server-github"]',
    ...
    try {
      ...
      assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
      assert.match(result.stdout, /mcp_servers\.memory \(deprecated; use MAMGA via vibestart\.ps1\)/);
...
if (
  test('sync installs the missing Codex baseline and accepts the legacy context7 MCP section', () => {
    ...
    try {
      ...
      assert.ok(!Object.prototype.hasOwnProperty.call(parsedConfig.mcp_servers, 'memory'));
