...
// The generic server-memory MCP is fragile in Codex startup on Windows and is
// superseded for this repo by MAMGA, which is started by scripts/vibestart.ps1.
const DEPRECATED_SERVERS = ['memory'];
...
function main() {
  ...
  for (const name of DEPRECATED_SERVERS) {
    if (existing[name]) {
      toRemoveLog.push(`mcp_servers.${name} (deprecated; use MAMGA via vibestart.ps1)`);
      raw = removeServerFromText(raw, name, existing);
      log(`  [remove] mcp_servers.${name} (deprecated; use MAMGA via vibestart.ps1)`);
    }
  }
